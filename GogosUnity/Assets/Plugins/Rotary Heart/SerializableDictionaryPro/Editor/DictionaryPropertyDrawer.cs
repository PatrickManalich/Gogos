...
namespace RotaryHeart.Lib.SerializableDictionaryPro
{
    ...
    public class DictionaryPropertyDrawer : PropertyDrawer
    {
        ...
        SerializedProperty m_keysValues;

        DrawableDictionary m_target;
        ReorderableList m_list;
        SerializedProperty m_val;
        ...
        {
            m_val = property.FindPropertyRelative("m_val");
            int val = m_val.intValue;
            m_canAdd = (val & 1) == 1;
            m_canRemove = (val & 2) == 2;
            ...
            //Clone the serialized object and use it for the key values
            ...
            m_keysValues = clonedProperty.FindPropertyRelative("m_keys");
            ...
            m_list = (GetTargetObjectOfProperty(property) as DrawableDictionary).reorderableList;
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (m_drawAdd)
            {
                ...
                if (m_keyAdd.GetArrayElementAtIndex(0).isExpanded)
                {
                    offset += EditorGUI.GetPropertyHeight(m_keyAdd.GetArrayElementAtIndex(0));
                }
            }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (m_valuesProp != null)
            {
                if (m_valuesProp.arraySize != m_keysProp.arraySize)
                    m_valuesProp.arraySize = m_keysProp.arraySize;
            }
            if (m_keysValues.arraySize != m_keysProp.arraySize)
                m_keysValues.arraySize = m_keysProp.arraySize;
            ...
            if (m_list != null)
            {
                ...
                m_list.isExpanded = m_val.isExpanded;
            ...
            if (m_drawAdd)
            {
                float keyHeight = EditorGUI.GetPropertyHeight(m_keyAdd.GetArrayElementAtIndex(0));
                ...
                switch (m_keyAdd.GetArrayElementAtIndex(0).propertyType)
                {
                    case SerializedPropertyType.Enum:
                        SerializedProperty keyToUse = m_keyAdd.GetArrayElementAtIndex(0);
                        ...
                        keyToUse.enumValueIndex = (int)m_addValue;
                        
                        string selectedVal = names[(int)m_addValue];
                    ...
                    default:
                        ...
                        {
                            SetPropertyValue(m_keyAdd.GetArrayElementAtIndex(0), m_addValue);
                            
                            EditorGUI.PropertyField(keyRect, m_keyAdd.GetArrayElementAtIndex(0), Style.keyAddContent,
                                true);

                            if (changeCheckScope.changed)
                            {
                                m_addValue = GetPropertyValue(m_keyAdd.GetArrayElementAtIndex(0));
                ...
                if (GUI.Button(rect, "Add") && m_addValue != null)
                {
                    ...
                    int index = m_keysValues.arraySize;

                    // object newKey = GetPropertyValue(m_keyAdd.GetArrayElementAtIndex(0));
                    
                    int count;
                    if (ContainsId(m_addValue, index, out count))
                    {
                        // if (count >= 1)
                        //     Remove(index);
                        m_drawErrorOnAdd = true;
                        return;
                    }
                    ...
                    if (m_valuesProp != null)
                        m_valuesProp.arraySize = m_keysValues.arraySize;

                    m_keysValues.serializedObject.ApplyModifiedProperties();
                    ...
                    // else
                    // {
                        SetPropertyValue(m_keysValues.GetArrayElementAtIndex(index), m_addValue);
                        SetPropertyValue(m_keysProp.GetArrayElementAtIndex(index), m_addValue);

                        if (m_valuesProp != null)
                        {
                            var propObj = GetTargetObjectOfProperty(m_valuesProp.GetArrayElementAtIndex(index));
                            ...
                            if (test == typeof(DrawableDictionary))
                            {
                                ...
                                // element.FindPropertyRelative("m_keys").arraySize = 0;
                                //
                                // SerializedProperty vals = element.FindPropertyRelative("m_values");
                                // if (vals != null)
                                // {
                                //     vals.arraySize = 0;
                                // }
                    ...
                    if (m_target.EditorOnAdded != null)
                        m_target.EditorOnAdded.Invoke(m_keysProp.GetArrayElementAtIndex(index), m_valuesProp?.GetArrayElementAtIndex(index));
                    
                    m_keyAdd.arraySize = 0;
        ...
        void CreateList(SerializedProperty property)
        {
            ...
            (GetTargetObjectOfProperty(property) as DrawableDictionary).reorderableList = m_list;
        }
        ...
        {
            m_keyAdd.arraySize = 1;
            m_keyAdd.GetArrayElementAtIndex(0).isExpanded = false;
            SetPropertyValue(m_keyAdd.GetArrayElementAtIndex(0),
                             GetDefaultKeyValue(m_keyAdd.GetArrayElementAtIndex(0)));
            m_drawAdd = true;
            m_addValue = null;

            if (m_keyAdd.GetArrayElementAtIndex(0).propertyType == SerializedPropertyType.Enum)
            {
                int index = 0;

                List<int> numbersUsed = new List<int>();

                for (int i = 0; i < m_keysValues.arraySize; i++)
                    numbersUsed.Add(m_keysValues.GetArrayElementAtIndex(i).enumValueIndex);
                ...
                if (index >= m_keyAdd.GetArrayElementAtIndex(0).enumNames.Length)
                    index = 0;

                m_keyAdd.GetArrayElementAtIndex(0).enumValueIndex = index;
            }
        }
        ...
        void List_onDrawElement(Rect rect, SerializedProperty element, GUIContent label, int index, bool selected, bool focused)
        {
            ...
            SerializedProperty keyValueProp = m_keysValues.GetArrayElementAtIndex(index);
            ...
            // Debug.Log(keyValueProp.propertyPath);
            // Debug.Log(keyProp.propertyPath);
            // Debug.Log("element: " + element.stringValue + ", " + element.arraySize);
            // Debug.Log("keyValueProp: " + keyValueProp.stringValue + ", " + keyValueProp.arraySize);
            // Debug.Log("keyProp: " + keyProp.stringValue + ", " + keyProp.arraySize);
            ...
            switch (keyPropType)
            {
                case SerializedPropertyType.Quaternion:
                    ...
                    {
                        Vector4 newV4 =
                            EditorGUI.Vector4Field(keyRect, GUIContent.none,
            ...
            if (valueProp != null)
            {
                ...
                if (valPropType == SerializedPropertyType.Generic)
                {
                    ...
                    using (EditorGUI.ChangeCheckScope changeCheckScope = new EditorGUI.ChangeCheckScope())
                    {
                        ...
                        {
                            using (new EditorGUI.PropertyScope(valueRect, GUIContent.none, valueProp))
                            {
                                if (valPropType == SerializedPropertyType.Quaternion)
                                {
                                    using (EditorGUI.ChangeCheckScope changeCheckScope2 =
                                        new EditorGUI.ChangeCheckScope())
                                    {
                                        Vector4 newV4 = EditorGUI.Vector4Field(valueRect, GUIContent.none,
                                                                               QuaternionToVector4(
                                                                                   valueProp.quaternionValue));
                ...
                else
                {
                    ...
                    if (keyProp.isExpanded)
                    {
                        ...
                        using (new EditorGUI.PropertyScope(valueRect, GUIContent.none, valueProp))
                        {
                            EditorGUI.PrefixLabel(valueRect, Style.valueContent);
                            if (valPropType == SerializedPropertyType.Quaternion)
                            {
                                using (EditorGUI.ChangeCheckScope changeCheckScope = new EditorGUI.ChangeCheckScope())
                                {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (m_target.EditorOnEdit != null)
                    m_target.EditorOnEdit.Invoke(keyProp, valueProp);
            }
        ...
        void List_onDrawHeader(Rect rect, GUIContent label)
        {
            ...
            using (EditorGUI.ChangeCheckScope changeCheckScope = new EditorGUI.ChangeCheckScope())
            {
                m_val.isExpanded = EditorGUI.Foldout(rect, m_val.isExpanded, string.Empty, true);
                
                if (changeCheckScope.changed)
                    m_val.serializedObject.ApplyModifiedProperties();
            }

            EditorGUI.LabelField(rect, m_title + (Constants.ShowSize ? " [" + m_keysValues.arraySize.ToString("N0") + "]" : string.Empty));
        }
        ...
        float List_onGetElementHeight(SerializedProperty element, int index)
        {
            ...
            //Value height
            if (!element.isExpanded || m_valuesProp == null || m_valuesProp.arraySize <= index) return height;
        ...
        {
            m_keysValues.MoveArrayElement(startIndex, newIndex);
